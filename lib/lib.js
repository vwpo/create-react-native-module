...
` + (generateExample
  ...
  exampleSymlinkDisabled: ${exampleSymlinkDisabled}
      ...
      return Promise.resolve()
        ...
        .then(() => {
          ...
          return new Promise((resolve, reject) => {
            ...
            // postinstall workaround script only needed in case of
            // the **deprecated** `exampleSymlinkDisabled: true`
            // setting (not needed otherwise):
            if (exampleSymlinkDisabled) {
              // (adding the cleanup postinstall *workaround* task to the example scripts)
            ...
            const yarnLinkCommand = exampleSymlinkDisabled
              ? 'yarn add file:../'
              : 'yarn add link:../';
            const addLinkLibraryOptions = { cwd: pathExampleApp, stdio: 'inherit' };
            try {
              commandSync(yarnLinkCommand, addLinkLibraryOptions);
            } catch (e) {
